<?php
    ...
    echo "First Name" .$_POST["q72_name72"]["first"] . " ";
    $firstName = $_POST["q72_name72"]["first"];
     
    echo "Last Name" .$_POST["q72_name72"]["last"] . " ";
    $lastName = $_POST["q72_name72"]["last"]; 
    
    /*
        $firstName = $_POST["q72_name72"]["first"];
        $lastName = $_POST["q72_name72"]["last"]; 
        $email = $_POST["q73_email73"];
        $date = $_POST["q48_whenDid48"]["year"] . "-" . $_POST["q41_whenDid"]["month"] . "-" . $_POST["q41_whenDid"]["day"];
        $amount = $_POST["q49_howMuch49"]; 
        $notes = $_POST["q142_instructionsTo"] . $_POST["q51_whyAre51"];
        $file_name = $_POST["temp_upload"]["q50_uploadA50"][0];
    */
...
$mysqli = new mysqli("127.0.0.1", "vincerap7132_testing1", "Test013020#", "vincerap7132_testing");
    ...
    if (trim($_POST["q72_name72"]["first"]) == NULL){
        $required = $required + 1;
    }
    
    if (trim($_POST["q72_name72"]["last"]) == NULL){
        $required = $required + 1;
    }
    
    if (trim($_POST["q73_email73"]) == NULL){
        $required = $required + 1;
    }
    
     if (trim($_POST["q48_whenDid48"]["month"]) == NULL){
        $required = $required + 1;
    }
    
     if (trim($_POST["q48_whenDid48"]["day"]) == NULL){
        $required = $required + 1;
    }
    
     if (trim($_POST["q48_whenDid48"]["year"]) == NULL){
        $required = $required + 1;
    }
    
     if (trim($_POST["q49_howMuch49"]) == NULL){
        $required = $required + 1;
    }
    
    if (trim($_POST["q142_instructionsTo"]) == NULL){
        $required = $required + 1;
    }
    
     if (trim($_POST["temp_upload"]["q50_uploadA50"][0]) == NULL){
        $required = $required + 1;
    }
